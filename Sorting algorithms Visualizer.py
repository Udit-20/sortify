...
RECTANGLE_COLOR = (255, 255, 0)  # Changed from blue to yellow
BORDER_COLOR = (0, 0, 0)
...
def draw_rectangles(arr, index1=None, index2=None):
    screen.fill(WHITE)
    rect_width = 20
    gap = 40
    start_x = (screen.get_width() - (len(arr) * (rect_width + gap) - gap)) // 2
    for i, val in enumerate(arr):
        x = start_x + i * (rect_width + gap)
        y = screen.get_height() // 2 - val
        color = RECTANGLE_COLOR
        if i == index1 or i == index2:
            color = LIGHT_BLUE
        elif i > len(arr) - len(arr[i:]):
            color = GREEN
        pygame.draw.rect(screen, color, (x, y, rect_width, val))
        text_surface = font.render(str(val), True, BUTTON_TEXT_COLOR)
        text_rect = text_surface.get_rect(center=(x + rect_width // 2, y - 10))
        screen.blit(text_surface, text_rect)
        if i == index1 or i == index2:
            pygame.draw.rect(screen, RED, (x, y, rect_width, val), width=2)
    draw_buttons()
